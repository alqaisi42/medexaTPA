...
export async function GET(request: NextRequest) {
    ...
    try {
        return await forwardRequest("/api/v1/icd-codes", undefined, searchParams)
    } catch (error) {
        console.error("Failed to proxy ICD list request", error)
        return new Response(JSON.stringify({ message: "Failed to load ICD codes" }), {
            status: 500,
...
export async function POST(request: NextRequest) {
    ...
    try {
        return await forwardRequest("/api/v1/icd-codes", {
            method: "POST",
        ...
        console.error("Failed to proxy ICD create request", error)
        return new Response(JSON.stringify({ message: "Failed to create ICD code" }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        })
    }
