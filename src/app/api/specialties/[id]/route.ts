...
function buildUpstreamUrl(id: string) {
    return new URL(`/api/v1/specialties/${id}`, apiBaseUrl).toString()
}
...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        console.error("Failed to proxy specialty update request", error)
        return new Response(JSON.stringify({ message: "Failed to update specialty" }), {
            status: 500,
...
export async function DELETE(_: NextRequest, context: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        console.error("Failed to proxy specialty delete request", error)
        return new Response(JSON.stringify({ message: "Failed to delete specialty" }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        })
    }
