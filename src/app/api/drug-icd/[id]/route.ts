...
function buildUpstreamUrl(id: string) {
    return new URL(`/api/v1/drug-icd/${id}`, apiBaseUrl).toString()
}
...
export async function GET(
    ...
    try {
        ...
        console.error("Failed to proxy drug ↔ ICD relations fetch request", error)
...
export async function PUT(
    ...
    try {
        ...
        console.error("Failed to proxy drug ↔ ICD relation update request", error)
        return new Response(JSON.stringify({ message: "Failed to update ICD relation" }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        })
    }
...
export async function DELETE(
    ...
    try {
        ...
        console.error("Failed to proxy drug ↔ ICD relation delete request", error)
        return new Response(JSON.stringify({ message: "Failed to delete ICD relation" }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        })
    }
