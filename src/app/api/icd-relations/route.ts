...
export async function GET(request: NextRequest) {
    ...
    try {
        return await forwardRequest("/api/v1/icd-relations", undefined, searchParams)
    } catch (error) {
        console.error("Failed to proxy ICD relations list request", error)
...
export async function POST(request: NextRequest) {
    ...
    try {
        return await forwardRequest("/api/v1/icd-relations", {
            method: "POST",
        ...
        console.error("Failed to proxy ICD relation create request", error)
        return new Response(JSON.stringify({ message: "Failed to create ICD relation" }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        })
    }
