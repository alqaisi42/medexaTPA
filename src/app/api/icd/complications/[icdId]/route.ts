...
function buildUpstreamUrl(icdId: string) {
    return new URL(`/api/v1/icd/complications/${icdId}`, apiBaseUrl).toString()
}
...
export async function GET(
    ...
    try {
        ...
        console.error("Failed to proxy ICD complication list request", error)
        return new Response(JSON.stringify({ message: "Failed to load ICD complications" }), {
            status: 500,
